...
(function(kcharts, $, _) {
    ...
    kcharts.Simple2D = function() {
        ...
        // OUR SINGLE LINE
        var X = function(d) {return x(+d.x);};
        var Y = function(d) {return y(+d.y);};
        var line = d3.svg.line().x(X).y(Y);
        // heatmap
        var heatmap = new kcharts.HeatMap().xScale(x).yScale(y).radius(0.02);
        ...
        function chart(selection) {
            selection.each(function(data) {
                ...
                // UPDATE THE AXES 
                g.select('.x.axis')
                    .attr("transform", "translate(0," + y.range()[0] + ")").call(xAxis);
                    //.transition().duration(1000).call(xAxis);
                g.select('.y.axis').call(yAxis);//.transition().duration(1000).call(yAxis);
